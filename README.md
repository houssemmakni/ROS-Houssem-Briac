...
**Objectif** : Survivez le plus de vagues possible en eliminant tous les zombies avant d'avoir perdu vos 100 PV.

**Regles** :
- Le robot joueur possede **100 points de vie** (barre de PV)
...
- Une nouvelle vague de zombies apparait toutes les **15 secondes**
- Chaque vague apporte plus de zombies avec plus de PV
- A partir de la **vague 5**, certains zombies utilisent le lance-flamme
...
# 3. Installer le projet et ses dependances
pip install -e .
```

L'installation via `pip install -e .` installe automatiquement **pygame** et **numpy**.
...
```bash
# Activation du venv (si pas deja fait)
